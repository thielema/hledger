...
- item order: security fixes come first, then breaking changes, then the rest. Within a section, order items by importance to users, and group related items together (eg appearance changes, then behaviour changes, then packaging). In the project changelog, the Docs and Tools/infrastructure lists are sorted alphabetically.
- section headings, in package changelogs: when there are several security-related items (fixes or hardening), group them all under a "Security" heading, which comes first. "Breaking changes" comes next, when there are any (the draft provides it). After that, use topic headings for a long section (as in hledger's recent releases: ## Lot tracking, ## Reports, ## Data import etc), or a suitable generic heading or two otherwise (eg Fixes, Improvements) - or none, for a very short section. Don't use the old fixed six-heading template, and never leave empty headings.
