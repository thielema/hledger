...
import Hledger.Write.Journal (journalItemsAsText)
...
printmode = hledgerCommandMode
  ...
  ,flagNone ["export"] (setboolopt "export") "reproduce the whole journal, with directives, comments and blank lines preserved and included files inlined"
  ,let arg = "DESC" in
...
printEntries opts@CliOpts{rawopts_=rawopts, reportspec_=rspec} j =
  ...
  where
    ...
    -- With --export, reproduce the journal file(s): directives, comments and blank lines verbatim,
    -- and the (filtered, processed) transactions in their original positions.
    -- Anything generated by hledger (--forecast, --auto, --infer-equity) is excluded.
    renderExport
      | fmt=="txt" = journalItemsAsText showtxn (jitems j) . styleAmounts styles . dropGenerated . map maybeoriginalamounts
      | otherwise  = error' "print --export supports only the txt output format"  -- PARTIAL:
      where
        -- the blank line after each transaction is a separate journal item
        showtxn t = let s = showTransactionWithLayout postinglayout t in fromMaybe s $ T.stripSuffix "\n" s
        dropGenerated = map dropGeneratedPostings . filter (not . transactionHasTag generatedTransactionTagName)
        dropGeneratedPostings t = t{tpostings = filter (not . postingHasTag generatedPostingTagName) $ tpostings t}
        transactionHasTag name = any ((==name) . fst) . ttags
